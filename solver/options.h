...
#define OPTSTEMPLATE \
  ...
  OPT (cutoff,50000,0,INT_MAX,"flips per try (0 = unlimited; controls inner-restart and shared-cache cycle frequency)"); \
  ...
  OPT (randk,0,0,INT_MAX,"random-source top-K: when the random-source path is taken (clsselectp roll or max-weight fallback), instead of picking one random satisfied source, randomly sample R*K satisfied sources with weight > min-weight (R = --randtour), take the top K by weight, and transfer from each with amount/K. K=0 (default) = original single-random behavior."); \

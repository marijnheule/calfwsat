...
#define OPTSTEMPLATE \
  ...
  OPT (randk,1,0,INT_MAX,"random-source top-K: when the random-source path is taken (clsselectp roll or max-weight fallback), instead of picking one random satisfied source, randomly sample R*K satisfied sources with weight > min-weight (R = --randtour), take the top K by weight, and transfer from each with amount/K. K=0 = original single-random behavior; K=1 (default, with randtour=3) = tournament-of-3 heaviest-source selection, which benchmarked best on ntil-43/ntil-45."); \
  OPT (randtour,3,1,INT_MAX,"random-source tournament size multiplier: when --randk=K>0, sample R*K satisfied sources before picking the top K by weight. Larger R biases the picked sources toward heavier ones at the cost of more rejection sampling per transfer."); \
  OPT (wsample,0,0,1,"weighted-proportional random source sampling: draw each random-source candidate from a segment sum-tree over ddfw weights (prob proportional to weight) instead of a uniform index. SAT/min-weight/hard-soft are rejection-filtered at draw time, so the tree carries weight only and updates just 2 leaves per transfer."); \
  OPT (wsamplepow,1,1,8,"--wsample draw exponent p: probability proportional to weight^p (1 = linear, 2 = quadratic, ...). Higher p concentrates the draw on heavier sources. Only affects the draw distribution, not the transfer amount or top-K selection (both still use the linear weight)."); \

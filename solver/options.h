...
#define OPTSTEMPLATE \
  ...
  OPT (wsamplepow,1,0,8,"weighted-proportional random source sampling exponent p: draw each random-source candidate from a segment sum-tree with probability proportional to ddfw weight^p, instead of a uniform index. p=0 disables it (original uniform sampling); p=1 (default) linear; p=2 quadratic; higher p concentrates harder on heavy sources. SAT/min-weight/hard-soft are rejection-filtered at draw time, so the tree carries weight^p only and updates just 2 leaves per transfer. Only the draw distribution changes -- the transfer amount and top-K selection still use the linear weight."); \

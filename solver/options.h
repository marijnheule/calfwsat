...
#define OPTSTEMPLATE \
  ...
  OPT (cutoff,1000,0,INT_MAX,"flips per try (0 = unlimited)"); \
  OPT (dynmul,10,0,INT_MAX,"extend cutoff to D*probe-flips on improvement (0=off)"); \
  OPT (boost,10,0,INT_MAX,"tiered cutoff boost: at each N^k*cutoff, extend by *N iff probe's best < avg best of probes reaching that tier (0/1=off)"); \
  ...
  OPT (inject,8,0,INT_MAX,"weight-transfer inject: if 0<=w<=N, sink gains N not w, raising total weight (0=off)"); \
  ...
  OPT (min_gain,100,0,INT_MAX,"snap |flip_gain| < val/1000 to 0 (0 = off)"); \

...
#define OPTSTEMPLATE \
  ...
  OPT (randk,0,0,INT_MAX,"random-source top-K: when the random-source path is taken (clsselectp roll or max-weight fallback), instead of picking one random satisfied source, randomly sample until 3N satisfied sources are collected, take the top N by weight, and transfer from each with amount/N. N=0 (default) = current single-random behavior."); \

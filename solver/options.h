...
#define OPTSTEMPLATE \
  ...
  OPT (litheap,0,0,1,"maintain per-literal max-weight neighbor heaps to speed up max-weight source selection"); \
  ...
  OPT (wtadd,167,0,1000,"weight transfer additive term c (c = wtadd/1000)"); \
  ...
  OPT (init_clause_weight,100,1,INT_MAX,"initial ddfw clause weight"); \
  OPT (init_card_weight,100,1,INT_MAX,"initial ddfw cardinality constraint weight"); \
  ...
  OPT (maxs_init_weight_relative,1,0,1,"ddfw weights for soft constraints start as MaxSAT weights"); \
